...
# - http://sysplan.nams.kyushu-u.ac.jp/gen/edu/SystemsDesignEngineering/2017/kougi03/kougi03.pdf
...
def calc_2d_spline_interpolation(x, y, num=100):
    ...
    # coefficients: 
    # a_x: sp.sx.a, b_x: sp.sx.b, c_x: sp.sx.c, d_x: sp.sx.d
    # a_y: sp.sy.a, b_y: sp.sy.b, c_y: sp.sy.c, d_x: sp.sy.d

    r_x, r_y, r_yaw, r_k = [], [], [], []
    coef = np.zeros((len(s),4,2)) # 4: a, b, c, d; 2: x, y;

    ### reference point loop
    for i_p in range(len(sp.sx.b)):
        # print(i_p)
        coef[i_p,:,0] = [sp.sx.a[i_p], sp.sx.b[i_p], sp.sx.c[i_p], sp.sx.d[i_p]]
        coef[i_p,:,1] = [sp.sy.a[i_p], sp.sy.b[i_p], sp.sy.c[i_p], sp.sy.d[i_p]]

    ### num(from attribute) loop
    for i_s in s:
    ...
    return r_x, r_y, r_yaw, r_k, travel, coef, sp.s, sp
...
def test_spline2d():
    ...
    x, y, yaw, k, travel, _, _, _ = calc_2d_spline_interpolation(input_x, input_y, num=200)
...
def test_spline():
    ...
    # y = [5.0, 3.5, 4.0, 5.5, 7.0, 10.5, 9.0, 8.3, 6.7, 8.8, 5.0, 2.0, 0.0]
    # y = np.sin(x)
